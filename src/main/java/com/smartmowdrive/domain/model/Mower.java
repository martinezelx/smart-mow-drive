...
public class Mower {
    ...
    public void turnLeft() {
        ...
        log.info(String.format("Mower %s turned left and this heading is pointing to %s", this.id, this.position));
    }
    ...
    public void turnRight() {
        ...
        log.info(String.format("Mower %s turned right and this heading is pointing to %s", this.id, this.position));
    }
    ...
    public void moveForward(List<MowerFinalPosition> mowersFinalPositions) {
        ...
        for (MowerFinalPosition mowerFinalPosition : mowersFinalPositions) {
            if (mowerFinalPosition.x() == newX && mowerFinalPosition.y() == newY) {
                ...
                log.error(String.format("Mower %s try to move forward to X:%s Y:%s but the position is occupied", this.id, newX, newY));
                return;
            }
        }
        ...
        this.x = newX;
        this.y = newY;
        log.info(String.format("Mower %s move forward and this new position is X:%s Y:%s", this.id, this.x, this.y));
    }
}
